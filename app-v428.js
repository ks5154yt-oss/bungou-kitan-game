...
try{
  window.__BK_BUILD__="V428";
  localStorage.setItem("bungou_kitan_build","V428");
}catch(_){}
...
function shell(x){
 ...
 A.innerHTML=`${premium?"":`<div class=top><b>文豪綺譚 <span class=gold>V428</span></b><small>EXP ${S.xp} / 🖋️${S.ink}</small></div>`}<div class=screenFade>${x}</div>${premium?"":nav()}`
}
...
function toggleAutoBattleV122(){
 ...
 try{posterAutoSyncV230?.()}catch(_){};setTimeout(()=>placeAutoButtonV428(),0)
}


let __autoButtonRafV428=0;

function removeAutoButtonV428(){
 let x=document.getElementById("autoButtonV428");
 if(x)x.remove();
 if(__autoButtonRafV428){cancelAnimationFrame(__autoButtonRafV428);__autoButtonRafV428=0}
}

function placeAutoButtonV428(){
 let btn=document.getElementById("autoButtonV428");
...
function installAutoButtonV428(){
 removeAutoButtonV428();
 ...
 btn.addEventListener("pointerdown",fire,{capture:true});
 ...
 const follow=()=>{
   if(!document.body.classList.contains("battleMode")){
     removeAutoButtonV428();return;
   }
   placeAutoButtonV428();
   __autoButtonRafV428=requestAnimationFrame(follow);
 };
 __autoButtonRafV428=requestAnimationFrame(follow);
}
...
function mountBattleV182(){
  ...
  setTimeout(()=>installAutoButtonV428(),60);if(ensureBattleStateV120().auto)setTimeout(()=>startAutoCoreV427(),120);
  return markup
}
...
function leaveBattleCleanupV191(){removeAutoButtonV428?.();clearPosterQueueV230();posterRendererResetV229();posterStageResetV228();flushBattleSaveV197();battleInvalidateActionsV195();clearTimeout(__battleGovernorTimerV193);__battleLongTaskCountV193=0;
 stopAutoBattleV122?.();
 clearBattleTimersV191();
 purgeBattleTransientDomV191();
 pauseDecorativeFxV190?.(true);
