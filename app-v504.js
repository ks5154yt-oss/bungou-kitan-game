...
try{
  window.__BK_BUILD__="V504";
  localStorage.setItem("bungou_kitan_build","V504");
}catch(_){}
...
function shell(x){
 ...
 A.innerHTML=`${premium?"":`<div class=top><b>文豪綺譚 <span class=gold>V504</span></b><small>EXP ${S.xp} / 🖋️${S.ink}</small></div>`}<div class=screenFade>${x}</div>${premium?"":nav()}`
}
...
function dedicatedHome(){
 ...
 shell(`<main class=premiumHomeV236>
   ...
   <div class=premiumHomeReadyBadgeV499>V504 RANK</div>
...
function formationPickerV136(slot){
 ...
 <div class=formationCurrentV500>${(S.sets?.[S.set]||[]).slice(0,6).map((i,n)=>`<span class="${n===slot?"target":""}"><img src="${characterPortraitV501(i)}" style="${portraitStyleV502(i)}" alt="${C[i][1]}"><b>${n+1}</b></span>`).join("")}</div>
 ...
 <div class=formationPickerListV500>${a.map(i=>`<button class="${i===current?"current":""} ${team.has(i)?"inTeam":""}" data-formation-pick="${slot}:${i}"><img src="${characterPortraitV501(i)}" style="${portraitStyleV502(i)}" alt="${C[i][1]}"><div><span>${rankBadgeV504(i)} ${characterAbility(i).role}</span><b>${C[i][1]}</b><small>${C[i][2]}</small></div><aside><b>Lv.${lv(i)}</b><span>${unitPower(i).toLocaleString()}</span>${team.has(i)?`<em>編成中</em>`:""}</aside></button>`).join("")}</div></div>`
}
