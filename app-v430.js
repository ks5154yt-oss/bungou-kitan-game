...
try{
  window.__BK_BUILD__="V430";
  localStorage.setItem("bungou_kitan_build","V430");
}catch(_){}
...
function shell(x){
 ...
 A.innerHTML=`${premium?"":`<div class=top><b>文豪綺譚 <span class=gold>V430</span></b><small>EXP ${S.xp} / 🖋️${S.ink}</small></div>`}<div class=screenFade>${x}</div>${premium?"":nav()}`
}
...
function mountBattleV182(){
  ...
  setTimeout(()=>installAutoHardButtonV430(),80);if(ensureBattleStateV120().auto)setTimeout(()=>autoHardActionV430(),120);
  return markup
}
...
function leaveBattleCleanupV191(){document.getElementById("autoHardButtonV430")?.remove();autoHardStopV430?.();removeAutoButtonV429?.();clearPosterQueueV230();posterRendererResetV229();posterStageResetV228();flushBattleSaveV197();battleInvalidateActionsV195();clearTimeout(__battleGovernorTimerV193);__battleLongTaskCountV193=0;
 stopAutoBattleV122?.();
 clearBattleTimersV191();
 purgeBattleTransientDomV191();
 pauseDecorativeFxV190?.(true);
