...
try{
  window.__BK_BUILD__="V512";
  localStorage.setItem("bungou_kitan_build","V512");
}catch(_){}
...
function shell(x){
 ...
 A.innerHTML=`${premium?"":`<div class=top><b>文豪綺譚 <span class=gold>V512</span></b><small>EXP ${S.xp} / 🖋️${S.ink}</small></div>`}<div class=screenFade>${x}</div>${premium?"":nav()}`
}
...
function dedicatedHome(){
 ...
 shell(`<main class=premiumHomeV236>
   ...
   <div class=premiumHomeReadyBadgeV499>V512 SKILLS</div>
       ...
       return `<article class="formationEasyCardV509 ${i===current?"current":""} ${team.has(i)?"inTeam":""}">
         ...
         <div class=formationEasyMainV509>
           ...
           <div class=formationEasySkillsV509>
             <div><small>固有 / ${characterSkillKitV512(i).trait}</small><b>${characterSkillKitV512(i).skillName}</b></div>
