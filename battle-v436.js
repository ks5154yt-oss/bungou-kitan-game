window.__BK_BATTLE_BUILD__="V436";
...
(()=>{
  ...
  function isBoss(){try{return Number(ensureBattleStateV120()?.wave||0)===3}catch(_){return false}}
...
(()=>{
 ...
 function root(){return document.querySelector('.cleanBattleV238')||document.querySelector('.battleV119')}
 function stage(){const r=root();return r&&(r.querySelector('.posterActorStageV237')||r.querySelector('.bsv2Field_175')||r)}
 function boss(){try{return Number(ensureBattleStateV120()?.wave||0)===3}catch(_){return false}}
...
// V424: removed legacy V397 AUTO rescue; native AUTO is authoritative.
...
// V436 authoritative boss presentation tag.
(()=>{
  function syncBossWaveV436(){
    const root=document.querySelector('.cleanBattleV238');
    if(!root)return;
    let wave=0;
    try{wave=Number(ensureBattleStateV120()?.wave||0)}catch(_){}
    root.classList.toggle('v436BossWave',wave===3);
    if(wave!==3){
      root.querySelectorAll('.v387BossVeil,.v387BossIntroText,.v387BossSweep').forEach(x=>x.remove());
    }
  }
  syncBossWaveV436();
  const mo=new MutationObserver(()=>requestAnimationFrame(syncBossWaveV436));
  mo.observe(document.documentElement,{subtree:true,childList:true,characterData:true});
})();
