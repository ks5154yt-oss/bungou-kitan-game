...
try{
  window.__BK_BUILD__="V500";
  localStorage.setItem("bungou_kitan_build","V500");
}catch(_){}
...
function shell(x){
 ...
 A.innerHTML=`${premium?"":`<div class=top><b>文豪綺譚 <span class=gold>V500</span></b><small>EXP ${S.xp} / 🖋️${S.ink}</small></div>`}<div class=screenFade>${x}</div>${premium?"":nav()}`
}
...
function growth(){
 ...
 shell(`${uiPageHead("growth")}<main class=growthHubV483>
   ...
   <section class=growthRosterV483>
     ${growthPortraitsV483().map(n=>`<button class="${n===i?"active":""}" data-growth-char-v483="${n}"><img src="${characterImage(C[n][0])}" alt=""><span>${C[n][5]}</span><b>${C[n][1]}</b><small>Lv.${lv(n)}</small></button>`).join("")}
   </section>

   <section class=growthFocusV483>
     <div class=growthPortraitV483><img src="${characterImage(c[0])}" alt=""><span>${c[5]}</span></div>
...
function dedicatedHome(){
 ...
 shell(`<main class=premiumHomeV236>
   ...
   <div class=premiumHomeReadyBadgeV499>V499 QA</div>
...
function formationPickerV136(slot){
 ...
 return `<div class=formationPickerV500><header class=formationPickerHeadV500><div><small>FORMATION SELECT</small><b>${slot+1}枠目を変更</b><span>${a.length}人</span></div><button data-formation-picker-close=1>×</button></header>
 <div class=formationCurrentV500>${(S.sets?.[S.set]||[]).slice(0,6).map((i,n)=>`<span class="${n===slot?"target":""}"><img src="${characterImage(C[i][0])}"><b>${n+1}</b></span>`).join("")}</div>
 ...
 <div class=formationPickerListV500>${a.map(i=>`<button class="${i===current?"current":""} ${team.has(i)?"inTeam":""}" data-formation-pick="${slot}:${i}"><img src="${characterImage(C[i][0])}"><div><span>${charRank(i)} / ${characterAbility(i).role}</span><b>${C[i][1]}</b><small>${C[i][2]}</small></div><aside><b>Lv.${lv(i)}</b><span>${unitPower(i).toLocaleString()}</span>${team.has(i)?`<em>編成中</em>`:""}</aside></button>`).join("")}</div></div>`
}
