...
try{
  window.__BK_BUILD__="V530";
  localStorage.setItem("bungou_kitan_build","V530");
}catch(_){}
...
function shell(x){
 ...
 A.innerHTML=`${premium?"":`<div class=top><b>文豪綺譚 <span class=gold>V530</span></b><small>EXP ${S.xp} / 🖋️${S.ink}</small></div>`}<div class=screenFade>${x}</div>${premium?"":nav()}`
}
     ...
     ${rows.map(({c,i})=>{
       ...
       return `<article class="collectionCardV484 ${c[5]} ${owned?"owned":"unowned"}">
         <button class=collectionMainV484 data-char="${i}">
           <div class=collectionImgV484><img src="${characterPortraitV501(i)}" alt="">${rankBadgeV504(i)}${artStatusBadgeV529(i)}${S.favs.includes(i)?`<em>★</em>`:""}</div>
...
function dedicatedHome(){
 ...
 shell(`<main class=premiumHomeV236>
   ...
   <div class=premiumHomeReadyBadgeV499>V530 HARDEN</div>
