...
try{
  window.__BK_BUILD__="V460";
  localStorage.setItem("bungou_kitan_build","V460");
}catch(_){}
...
function shell(x){
 ...
 A.innerHTML=`${premium?"":`<div class=top><b>文豪綺譚 <span class=gold>V460</span></b><small>EXP ${S.xp} / 🖋️${S.ink}</small></div>`}<div class=screenFade>${x}</div>${premium?"":nav()}`
}
...
function finalBossClearGuardV460(){
 try{
   ...
   let allDead=hp.length>0&&hp.every(x=>(+x||0)<=0);
