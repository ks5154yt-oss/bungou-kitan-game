...
try{
  window.__BK_BUILD__="V535";
  localStorage.setItem("bungou_kitan_build","V535");
}catch(_){}
...
function shell(x){
 ...
 A.innerHTML=`${premium?"":`<div class=top><b>文豪綺譚 <span class=gold>V535</span></b><small>EXP ${S.xp} / 🖋️${S.ink}</small></div>`}<div class=screenFade>${x}</div>${premium?"":nav()}`
}
...
function stageReadinessV524(ch){
 let st=stageMeta(ch),team=S.sets?.[S.set]||[],power=unitPower(team),enemy=stageEnemyProfileV524(ch),coach=formationAnalysisV517(team),syn=skillSynergyV518(team);
 let ratio=st.power?power/st.power:1;
 let grade=ratio>=1.35?"余裕":ratio>=1.05?"適正":ratio>=.85?"挑戦":"危険";
 let pct=Math.max(0,Math.min(100,Math.round(ratio*78)));
 return {power,need:st.power,enemy,coach,syn,grade,pct,mode:enemy.mode}
}
function stagePrepPanelV524(ch){
 ...
 return `<section class=stagePrepPanelV524>
   ...
   <div class=stageIdentityV534><span>戦場</span><b>${battleStageIdentityV534(ch).area}</b><small>敵Lv目安 ${battleStageIdentityV534(ch).lv}〜 / ${ch===3?"最終章":"全3WAVE"}</small></div>
   ...
   <div class=stagePartyMiniHeadV535><span>BATTLE MEMBERS</span><b>${team.length}/6</b></div><div class=stagePartyMiniV533>${team.map((i,n)=>`<button data-char="${i}"><em>${n===0?"隊長":`#${n+1}`}</em><img src="${characterPortraitV501(i)}" alt="${C[i][1]}"><span>${C[i][1]}</span><small>${charRank(i)} / Lv.${lv(i)}</small></button>`).join("")}</div>
   <div class=stagePrepActionsV524>
   ...
   <div class=stagePrimarySortieV532>
     <button class=normal data-battle="${ch}" data-mode=normal><small>NORMAL</small><b>この章へ出撃</b><span>戦闘開始 ›</span></button>
     <button class=hard data-battle="${ch}" data-mode=hard ${!hardUnlocked(ch)?"disabled":""}><small>HARD</small><b>${hardUnlocked(ch)?"高難度へ出撃":`NORMALあと${Math.max(0,3-(S.progress?.clears?.[ch]||0))}回`}</b><span>${hardUnlocked(ch)?"戦闘開始 ›":"未解放"}</span></button>
   </div>
 </section>`
}
...
function stagePage(){
 ...
 shell(`${typeof uiPageHead==="function"?uiPageHead("sortie"):""}<div class="p stagePageV100"><div class=stageCompactHead><button data-go=sortie>‹ 一覧</button><div><small>第${i+1}章</small><b>${st.name}</b></div><span>★${stars}/3</span></div><button class=stageInstantSortieV532 data-battle="${i}" data-mode=normal><small>NORMAL</small><b>⚔️ この章へ出撃</b><span>タップで戦闘開始</span></button>${stagePrepPanelV524(i)}<div class=farmHeroV100><div class=farmHeroTitle><div><small>QUICK FARM</small><h1>周回する</h1></div><strong>${n}周</strong></div><div class=farmCountsV100>${[1,5,10,20,30,50].map(x=>`<button class="${n===x?"on":""}" data-stage-farm-count="${x}">${x}<small>周</small></button>`).join("")}</div><button class=farmExecuteV100 data-stage-farm-go="${i}" ${clear<1?"disabled":""}>⚡ ${clear<1?"初回クリアで周回解放":`${n}周を開始`}</button><div class=farmRemember>${clear<1?"NORMALを1回クリアすると周回できます":"前回の周回数を自動記憶・最大50周"}</div>${r&&r.ch===i?`<div class=farmLast><b>前回 ${r.count}周</b><span>原稿片 +${r.tickets||0}　EXP +${r.xp||0}　資料 +${r.mat||0}　文銭 +${r.gold||0}　装備 ${r.gear||0}</span></div>`:""}</div><div class="stageSubActions stageSubActionsV532"><button data-battle="${i}" data-mode=normal><b>NORMAL</b><small>通常戦闘</small></button><button data-battle="${i}" data-mode=hard ${!hardUnlocked(i)?"disabled":""}><b>HARD</b><small>${hardUnlocked(i)?"高難度":`NORMALあと${Math.max(0,3-clear)}回`}</small></button></div><div class=stageInfoV100><span>推奨<b>${st.power.toLocaleString()}</b></span><span>原稿片<b>${st.drop}</b></span><span>クリア<b>${clear}</b></span></div><div class=stageGoalsV524><span>★ クリア</span><span>★ 戦闘不能なし</span><span>★ 10ターン以内</span></div><div class=stageUnlockStatusV521><span>${i===3?(clear>0?"✓ メインストーリー全章踏破":"最終章"):(stageUnlockedV521(i+1)?"✓ 次章解放済":`次章解放条件：第${i+1}章 NORMALクリア`)}</span></div><div class=stageFlowChecklistV522><span class="${clear>0?"ok":""}">${clear>0?"✓":"○"} NORMAL初回クリア</span><span class="${hardUnlocked(i)?"ok":""}">${hardUnlocked(i)?"✓":"○"} HARD解放（NORMAL 3回）</span><span class="${i===3?(clear>0):stageUnlockedV521(i+1)?"ok":""}">${i===3?(clear>0?"✓":"○"):stageUnlockedV521(i+1)?"✓":"○"} ${i===3?"全章踏破":"次章解放"}</span></div>${stageRewardPanelV523(i)}<div class=stageNavV97><button data-stage-open="${Math.max(0,i-1)}" ${i===0?"disabled":""}>‹ 前章</button><button data-stage-open="${Math.min(3,i+1)}" ${(i===3||!stageUnlockedV521(i+1))?"disabled":""}>${i===3?"最終章":stageUnlockedV521(i+1)?"次章 ›":"🔒 次章"}</button></div></div>`)
}
function sortie(){
...
function dedicatedHome(){
 ...
 shell(`<main class=premiumHomeV236>
   ...
   <div class=premiumHomeReadyBadgeV499>V535 SIX</div>
...
function battleRewardV127(){
 ...
 return {mat:Math.round((30+b.wave*12)*scale),gold:Math.round((500+b.wave*250)*scale),ink:(stars>=3?2:1)+ch+(hard?2:0),stars}
}
function applyBattleRewardV127(){
 ...
 normalizeStageProgressV521();
 S.mat=(S.mat||0)+r.mat;S.gold=(S.gold||0)+r.gold;S.ink=(S.ink||0)+r.ink;
 if(mode==="hard"){
...
function posterSelectedCharV208(){
 let b=ensureBattleStateV120(),team=(S.sets?.[S.set]||[]).slice(0,6),slot=Math.max(0,Math.min(4,+b.unit||0)),i=+team[slot]||0;
...
if(ss524){
 ...
 toast(`${formationModeLabelV513(mode)}編成を作成しました`);
 return openStagePage(+ch)
}
