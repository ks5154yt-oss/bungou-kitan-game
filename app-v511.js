...
try{
  window.__BK_BUILD__="V511";
  localStorage.setItem("bungou_kitan_build","V511");
}catch(_){}
...
function shell(x){
 ...
 A.innerHTML=`${premium?"":`<div class=top><b>文豪綺譚 <span class=gold>V511</span></b><small>EXP ${S.xp} / 🖋️${S.ink}</small></div>`}<div class=screenFade>${x}</div>${premium?"":nav()}`
}
...
/* V511 legacy transition compatibility. */
function arenaEnsureV481(){
 ...
 if(!Number.isFinite(a.wins))a.wins=0;if(!Number.isFinite(a.losses))a.losses=0;if(!Number.isFinite(a.streak))a.streak=0;if(!Number.isFinite(a.bestStreak))a.bestStreak=0;if(!Number.isFinite(a.bestRating))a.bestRating=S.rating||1000;
 return a
}
function challenge(){toast("高難度は模擬戦へ統合しました");return go("arena")}
function tower(){toast("塔コンテンツは模擬戦へ統合しました");return go("arena")}
function exchange(){S.qol=S.qol||{};S.qol.summonTab="exchange";save();return go("summon")}
function daily(){toast("デイリー報酬はホームの報酬から確認できます");return go("home")}
function missions(){toast("ミッションはホームの報酬へ統合しました");return go("home")}
function profile(){return go("system")}
function settings(){return go("savevault")}
function event(){toast("イベント導線を出撃へ移動しました");return go("sortie")}
function storyScene(ch,n=0){try{let c=Math.max(0,Math.min(3,Number(ch)||0)),e=Math.max(0,Math.min(storyEpisodesV482(c).length-1,Number(n)||0));storyStateV482().tab="main";storyStateV482().chapter=c;save();story();return storyReaderV482(c,e)}catch(_){return go("story")}}
function workDetail(i){let n=Math.max(0,Math.min(WORKS.length-1,Number(i)||0)),name=WORKS[n]?.[0],ci=C.findIndex(c=>c[1]===name);if(ci>=0)return detail(ci);return go("list")}
...
function dedicatedHome(){
 ...
 shell(`<main class=premiumHomeV236>
   ...
   <div class=premiumHomeReadyBadgeV499>V511 STABLE</div>
...
function characterStatProfileV511(i){
 ...
 let roleMod={
   ...
   妨害:{hp:.96,atk:1.03,def:.96,spd:1.08,power:1.01},
   回復:{hp:1.08,atk:.86,def:1.05,spd:.96,power:.99},
   支援:{hp:1.02,atk:.92,def:1.00,spd:1.03,power:1.00},
   特殊:{hp:1.00,atk:1.05,def:1.00,spd:1.00,power:1.02},
   防御:{hp:1.16,atk:.90,def:1.20,spd:.90,power:1.03},
   速度:{hp:.90,atk:1.02,def:.89,spd:1.22,power:1.02}
 }[role]||{hp:1,atk:1,def:1,spd:1,power:1};
 let rankMod={UR:1.12,SSR:1.07,SR:1.03,R:1}[rank]||1;
 let variance=(n,span=.06)=>1+((((seed>>>n)%101)/100)-.5)*2*span;
 return {role,rank,hp:roleMod.hp*rankMod*variance(1,.045),atk:roleMod.atk*rankMod*variance(4,.055),def:roleMod.def*rankMod*variance(7,.055),spd:roleMod.spd*(1+(rankMod-1)*.45)*variance(10,.05),power:roleMod.power*rankMod*variance(13,.035)}
}
function characterStatsV511(i){
 let p=characterStatProfileV511(i),l=lv(i),gear=gearBonus(i)||{},m=masteryStats(i)?.total||0,sp=gearSpecialization(i)?.bonus||0;
 let amp=1+(m+sp)/100;
 return {hp:Math.round((880+l*42)*p.hp*amp),atk:Math.round((172+l*18+Number(gear.atk||0)*5)*p.atk*amp),def:Math.round((145+l*15+Number(gear.def||0)*3)*p.def*amp),spd:Math.round((96+l*.55)*p.spd),bias:p.power}
}
       ...
       return `<article class="formationEasyCardV509 ${i===current?"current":""} ${team.has(i)?"inTeam":""}">
         ...
         <div class=formationEasyMainV509>
           ...
           <div class=formationEasySkillsV509>
             <div><small>固有</small><b>${x.ability}</b></div>
             <div><small>奥義 ${o.gauge}</small><b>${o.name}</b></div>
           </div>
         </div>
