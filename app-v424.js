...
try{
  window.__BK_BUILD__="V424";
  localStorage.setItem("bungou_kitan_build","V424");
}catch(_){}
...
function shell(x){
 ...
 A.innerHTML=`${premium?"":`<div class=top><b>文豪綺譚 <span class=gold>V424</span></b><small>EXP ${S.xp} / 🖋️${S.ink}</small></div>`}<div class=screenFade>${x}</div>${premium?"":nav()}`
}
...
function scheduleAutoBattleV122(delay){
 stopAutoBattleV122();let b=ensureBattleStateV120();if(!b.auto)return;
 autoBattleTimerV122=setTimeout(runAutoBattleV122,delay??autoDelayV122())
}
function runAutoBattleV122(){
 let b=ensureBattleStateV120();
 if(!b.auto)return;
 if(autoBattleBusyV122||__battleActionBusyV195){
   if(!__nativeAutoPumpV424)scheduleAutoBattleV122(180);
   return;
 }
 ...
 try{
   ...
   posterBattleLockCueV214(kind);posterCommandToastV212(kind);posterReusableDirectorV228(slot,target,kind);posterStateCueV209(kind);posterActionCinematicV207(slot,target,kind);posterActionFeedbackV206(kind);battleActionFeedbackV204(slot,target,kind);safeAnimatedActionV130(kind,slot,target);
...
let __nativeAutoPumpV424=null;

function stopNativeAutoPumpV424(){
 if(__nativeAutoPumpV424){clearInterval(__nativeAutoPumpV424);__nativeAutoPumpV424=null}
}

function syncNativeAutoUiV424(){
 let b=ensureBattleStateV120();
...
function startNativeAutoPumpV424(){
 stopNativeAutoPumpV424();
 const tick=()=>{
   let b=ensureBattleStateV120();
   if(!b.auto||!document.body.classList.contains("battleMode")){
     stopNativeAutoPumpV424();return;
   }
   syncNativeAutoUiV424();
   if(!autoBattleBusyV122&&!__battleActionBusyV195&&!autoBattleTimerV122){
     runAutoBattleV122();
   }
 };
 __nativeAutoPumpV424=setInterval(tick,320);
 setTimeout(tick,80);
}
...
function resetAfterClearV130(){
 stopNativeAutoPumpV424?.();stopAutoBattleV122();let b=ensureAdvancedBattleV125();b.auto=false;b.combo=0;save();persistentSaveWrite()
}
...
function mountBattleV182(){
  ...
  if(ensureBattleStateV120().auto)setTimeout(()=>startNativeAutoPumpV424(),120);
  return markup
}
