...
try{
  window.__BK_BUILD__="V477";
  localStorage.setItem("bungou_kitan_build","V477");
}catch(_){}
...
function shell(x){
 ...
 A.innerHTML=`${premium?"":`<div class=top><b>文豪綺譚 <span class=gold>V477</span></b><small>EXP ${S.xp} / 🖋️${S.ink}</small></div>`}<div class=screenFade>${x}</div>${premium?"":nav()}`
}
...
function go(x){
 ...
 try{
  ...
  const routes={home,sortie,stagePage,party,list,summon,story,arena,growth};
  const aliases={
   characters:"list",character:"list",collection:"list",archive:"list",gear:"list",equipment:"list",
...
function dedicatedHome(){
 ...
 shell(`<main class=premiumHomeV236>
   ...
   <div class=premiumHomeReadyV477>
     <button data-go=sortie><small>進行</small><b>第${x.next+1}章</b></button>
     <button data-go=party><small>総戦力</small><b>${teamReadiness().power.toLocaleString()}</b></button>
     <button data-go=list><small>蒐集</small><b>${collectionProgress().pct}%</b></button>
     <button data-go=growth><small>育成</small><b>強化へ</b></button>
   </div>
