...
try{
  window.__BK_BUILD__="V547";
  localStorage.setItem("bungou_kitan_build","V547");
}catch(_){}
...
function shell(x){
 ...
 A.innerHTML=`${premium?"":`<div class=top><b>文豪綺譚 <span class=gold>V547</span></b><small>EXP ${S.xp} / 🖋️${S.ink}</small></div>`}<div class=screenFade>${x}</div>${premium?"":nav()}`
}
...
function stageFormationAuditV546(){
 let bad=[];
 for(const m of stageFormationModesV545()){
  let team=stageFormationPreviewV545(m.id);
  team.forEach(i=>{if(!selectableCharacterV531(i))bad.push({mode:m.id,i})})
 }
...
function stageFormationPreviewPanelV546(ch,mode){
 ...
 return `<div class=stageFormationPreviewV546>
   ...
   <div class=stageFormationPreviewCardsV546>
     ${team.map((i,n)=>`<div><i>${n===0?"隊長":n+1}</i><img src="${characterPortraitV501(i)}" alt="${C[i][1]}"><b>${C[i][1]}</b><small>${characterAbility(i).role} / ${charRank(i)}</small></div>`).join("")}
   </div>
...
function dedicatedHome(){
 ...
 shell(`<main class=premiumHomeV236>
   ...
   <div class=premiumHomeReadyBadgeV499>V547 FARM</div>
...
if(fr520){
 let [ch,n,mode]=String(fr520.dataset.farmRepeatV520).split(":");
 document.getElementById("farmResultV520")?.remove();
 if(!actionLock(500))return;
 return quickFarmRemembered(+ch,+n,mode||"normal")
}
...
let sfm545=e.target.closest("[data-stage-formation-mode-v545]");
if(sfm545){
 e.preventDefault();e.stopPropagation();
 let [ch,mode]=String(sfm545.dataset.stageFormationModeV545).split(":");
 S.qol=S.qol||{};S.qol.stageFormationModeV545=mode;
 save();
 return openStagePage(+ch)
}
let sfa545=e.target.closest("[data-stage-formation-apply-v545]");
if(sfa545){
 e.preventDefault();e.stopPropagation();
 let [ch,mode]=String(sfa545.dataset.stageFormationApplyV545).split(":");
 return applyStageFormationV545(+ch,mode)
}
let ss524=e.target.closest("[data-stage-smart-v524]");
if(ss524){
 let [mode,ch]=String(ss524.dataset.stageSmartV524).split(":");
 return applyStageFormationV545(+ch,mode||"balanced")
}
...
let fold544=e.target.closest(".stageDetailFoldV544");
if(fold544 && e.target.tagName==="SUMMARY"){
  setTimeout(()=>{
    S.qol=S.qol||{};
    S.qol.stageDetailOpenV544=!!fold544.open;
    save()
  },0)
}
