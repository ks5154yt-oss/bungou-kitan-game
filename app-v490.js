...
try{
  window.__BK_BUILD__="V490";
  localStorage.setItem("bungou_kitan_build","V490");
}catch(_){}
...
function shell(x){
 ...
 A.innerHTML=`${premium?"":`<div class=top><b>文豪綺譚 <span class=gold>V490</span></b><small>EXP ${S.xp} / 🖋️${S.ink}</small></div>`}<div class=screenFade>${x}</div>${premium?"":nav()}`
}
...
function growthExpNeedV483(i){
 let l=lv(i),cap=levelCap(i);if(l>=cap)return 0;
 return Math.max(10,Math.round(18+l*4.8))
}
function growthBatchCostV483(i,target){
 let l=lv(i),t=Math.min(levelCap(i),Math.max(l,target)),sum=0;
 for(let n=l;n<t;n++)sum+=Math.max(10,Math.round(18+n*4.8));
 return sum
}
function growthCanBreakV483(i){return lv(i)>=levelCap(i)&&levelCap(i)<150}
...
function growthLevelOneV483(i){
 ...
 S.xp-=need;S.levels[i]=Math.min(levelCap(i),lv(i)+1);return true
}
...
function growth(){
 ...
 let i=growthSelectedV483(),c=C[i],l=lv(i),cap=levelCap(i),need=growthExpNeedV483(i),avg=growthAvgBonusV483(),role=growthRoleV483(i),br=growthBreakCostV483(i);
...
setTimeout(()=>{try{
 let h=systemHealthV490();
 localStorage.setItem("bk_system_health_v490",JSON.stringify({at:Date.now(),pct:h.pct,build:window.__BK_BUILD__||"V490"}));
