...
try{
  window.__BK_BUILD__="V545";
  localStorage.setItem("bungou_kitan_build","V545");
}catch(_){}
...
function shell(x){
 ...
 A.innerHTML=`${premium?"":`<div class=top><b>文豪綺譚 <span class=gold>V545</span></b><small>EXP ${S.xp} / 🖋️${S.ink}</small></div>`}<div class=screenFade>${x}</div>${premium?"":nav()}`
}
...
function stageFormationSelectV545(ch){
 ...
 return `<section class=stageFormationSelectV545>
  ...
  <button class=stageFormationApplyV545 data-stage-formation-apply-v545="${ch}:${current}">この編成を適用</button>
 </section>`
}
...
function dedicatedHome(){
 ...
 shell(`<main class=premiumHomeV236>
   ...
   <div class=premiumHomeReadyBadgeV499>V545 FORM</div>
