...
try{
  window.__BK_BUILD__="V509";
  localStorage.setItem("bungou_kitan_build","V509");
}catch(_){}
...
function shell(x){
 ...
 A.innerHTML=`${premium?"":`<div class=top><b>文豪綺譚 <span class=gold>V509</span></b><small>EXP ${S.xp} / 🖋️${S.ink}</small></div>`}<div class=screenFade>${x}</div>${premium?"":nav()}`
}
...
function dedicatedHome(){
 ...
 shell(`<main class=premiumHomeV236>
   ...
   <div class=premiumHomeReadyBadgeV499>V509 EASY</div>
     ...
     ${a.map(i=>{
       ...
       return `<article class="formationEasyCardV509 ${i===current?"current":""} ${team.has(i)?"inTeam":""}">
         ...
         <div class=formationEasyActionsV509>
           <button class=detail data-char="${i}">詳細</button>
