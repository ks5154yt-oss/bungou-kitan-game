...
try{
  window.__BK_BUILD__="V538";
  localStorage.setItem("bungou_kitan_build","V538");
}catch(_){}
...
function shell(x){
 ...
 A.innerHTML=`${premium?"":`<div class=top><b>文豪綺譚 <span class=gold>V538</span></b><small>EXP ${S.xp} / 🖋️${S.ink}</small></div>`}<div class=screenFade>${x}</div>${premium?"":nav()}`
}
...
function stagePrepPanelV524(ch){
 ...
 return `<section class=stagePrepPanelV524>
   ...
   <div class=stagePartyMiniHeadV535><span>BATTLE MEMBERS</span><b>${team.length}/6</b></div><div class=stagePartyMiniV533 stagePartyMiniV538>${team.map((i,n)=>`<button data-char="${i}"><em>${n===0?"隊長":`#${n+1}`}</em><img src="${characterPortraitV501(i)}" alt="${C[i][1]}"><span>${C[i][1]}</span><small>${charRank(i)} / Lv.${lv(i)}</small></button>`).join("")}</div>
...
function dedicatedHome(){
 ...
 shell(`<main class=premiumHomeV236>
   ...
   <div class=premiumHomeReadyBadgeV499>V538 MOBILE</div>
