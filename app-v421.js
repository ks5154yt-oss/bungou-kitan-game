...
try{
  window.__BK_BUILD__="V421";
  localStorage.setItem("bungou_kitan_build","V421");
}catch(_){}
...
function shell(x){
 ...
 A.innerHTML=`${premium?"":`<div class=top><b>文豪綺譚 <span class=gold>V421</span></b><small>EXP ${S.xp} / 🖋️${S.ink}</small></div>`}<div class=screenFade>${x}</div>${premium?"":nav()}`
}
...
function runAutoBattleV122(){
 let b=ensureBattleStateV120();if(!b.auto||autoBattleBusyV122||__battleActionBusyV195)return;battleSessionActionV196();
 let token=battleActionGateV195("auto");if(!token)return;
...
function toggleAutoBattleV122(){
 let b=ensureBattleStateV120();b.auto=!b.auto;queueBattleSaveV197();smoothRefreshBattleV194();
 if(b.auto){scheduleAutoBattleV122(220);startAutoWatchdogV193()}else stopAutoBattleV122()
 posterAutoSyncV230();
}
...
function resetAfterClearV130(){
 stopAutoBattleV122();let b=ensureAdvancedBattleV125();b.auto=false;b.combo=0;save();persistentSaveWrite()
}
