...
try{
  window.__BK_BUILD__="V479";
  localStorage.setItem("bungou_kitan_build","V479");
}catch(_){}
...
function shell(x){
 ...
 A.innerHTML=`${premium?"":`<div class=top><b>文豪綺譚 <span class=gold>V479</span></b><small>EXP ${S.xp} / 🖋️${S.ink}</small></div>`}<div class=screenFade>${x}</div>${premium?"":nav()}`
}
...
function gearHubV479(){
 ...
 shell(`${uiPageHead("list")}<main class=gearHubV479>
   ...
   <section class=gearHubSummaryV479>
     ...
     <div><small>ロック</small><b>${stats.locked}</b></div>
...
function go(x){
 ...
 try{
  ...
  const routes={home,sortie,stagePage,party,list,summon,story,arena,growth,gear:gearHubV479};
  const aliases={
