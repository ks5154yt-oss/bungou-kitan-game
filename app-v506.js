...
try{
  window.__BK_BUILD__="V506";
  localStorage.setItem("bungou_kitan_build","V506");
}catch(_){}
...
function shell(x){
 ...
 A.innerHTML=`${premium?"":`<div class=top><b>文豪綺譚 <span class=gold>V506</span></b><small>EXP ${S.xp} / 🖋️${S.ink}</small></div>`}<div class=screenFade>${x}</div>${premium?"":nav()}`
}
...
function dedicatedHome(){
 ...
 shell(`<main class=premiumHomeV236>
   ...
   <div class=premiumHomeReadyBadgeV499>V506 INTEL</div>
...
function formationCompareV506(candidate,current){
 let cp=unitPower(candidate),op=unitPower(current);
...
function formationIntelV506(i,current){
 ...
 return {
   atk:charAtk(i),def:charDef(i),power:unitPower(i),
...
function formationPickerV136(slot){
 ...
 return `<div class=formationPickerV500>
   <header class=formationPickerHeadV500><div><small>FORMATION SELECT</small><b>${slot+1}枠目を変更</b><span>${a.length}人 / スキル・能力表示</span></div><button data-formation-picker-close=1>×</button></header>
   ...
   <div class=formationPickerListV500>${a.map(i=>{
     ...
     return `<button class="formationIntelCardV506 ${i===current?"current":""} ${team.has(i)?"inTeam":""}" data-formation-pick="${slot}:${i}">
