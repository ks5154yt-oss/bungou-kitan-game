...
try{
  window.__BK_BUILD__="V492";
  localStorage.setItem("bungou_kitan_build","V492");
}catch(_){}
...
function shell(x){
 ...
 A.innerHTML=`${premium?"":`<div class=top><b>文豪綺譚 <span class=gold>V492</span></b><small>EXP ${S.xp} / 🖋️${S.ink}</small></div>`}<div class=screenFade>${x}</div>${premium?"":nav()}`
}
...
function go(x){
 const __routeRequestedV492=x;
 if(document.body.classList.contains("battleMode"))leaveBattleCleanupV191();
 try{
  hideAutoReplayLoadingV199();
  if(x!=="stagePage")stopTrueAutoLoopV200(false);
  closeOverlays();
  document.body.classList.remove("battleMode");

  const routes={home,sortie,stagePage,party,list,summon,story,arena,growth,gear:gearHubV479,savevault:saveVaultV486,system:systemPanelV490};
  const aliases={
   characters:"list",character:"list",collection:"list",archive:"list",gear:"gear",equipment:"gear",
   formation:"party",team:"party",training:"growth",level:"growth",
   ...
   achievements:"home",goals:"home",dashboard:"home",save:"savevault",backup:"savevault",settings:"savevault",diagnostics:"system",status:"system"
  };

  x=aliases[x]||x;
  let fn=routes[x];

  // Unknown legacy routes should never strand the player or throw a "準備中" popup.
  if(typeof fn!=="function"){
    console.warn("V477 route fallback",x);
    x="home";fn=home;
  }

  window.__bkRoute=x;
  if(typeof syncNavActive==="function")syncNavActive(x);
  window.scrollTo(0,0);
  return renderScreen(x,fn)
 }catch(e){
  try{runtimeLogV491("ROUTE",String(__routeRequestedV492)+": "+(e?.message||e))}catch(_){}
  finalErrorShield(e);
