...
try{
  window.__BK_BUILD__="V515";
  localStorage.setItem("bungou_kitan_build","V515");
}catch(_){}
...
function shell(x){
 ...
 A.innerHTML=`${premium?"":`<div class=top><b>文豪綺譚 <span class=gold>V515</span></b><small>EXP ${S.xp} / 🖋️${S.ink}</small></div>`}<div class=screenFade>${x}</div>${premium?"":nav()}`
}
       ...
       return `<article class="collectionCardV484 ${c[5]} ${owned?"owned":"unowned"}">
         <button class=collectionMainV484 data-char="${i}">
           ...
           <div class=collectionInfoV484>
             ...
             <div class=collectionAbilityV484><b>${characterSkillKitV512(i).skillName}</b><span>${abilityDetail(i)}</span><small>${skillRankTextV512(i)}</small></div>
   ...
   <section class=formationSummaryV255>
     ...
     <section class=formationSmartV513>
       ...
       <div class=formationSmartButtonsV513>
         ...
         <button data-smartformation-v513="ougi">奥義回転</button>
       </div>
     </section>
   </section>
 </main>`)
...
function transitionAuditV508(){
 let tests=[
   ...
   ["lv",typeof lv==="function"],["arenaEnsure",typeof arenaEnsureV481==="function"],["storyScene",typeof storyScene==="function"],["legacyRoutes",typeof challenge==="function"&&typeof tower==="function"&&typeof exchange==="function"]
 ];
 ...
 shell(`${uiPageHead("growth")}<main class=growthHubV483>
   ...
   <section class=growthFocusV483>
     ...
     <div class=growthFocusTextV483>
       ...
       <div class=growthNeedV483>${l>=capLv?`現在の上限に到達`:`次Lvまで共有EXP ${need}`}</div><div class=growthSkillKitV512><b>${characterSkillKitV512(i).skillName}</b><span>${skillRankTextV512(i)}</span></div>
     </div>
...
function characterSkillKitV512(i){
 i=Math.max(0,Math.min(C.length-1,Number(i)||0));
 let a=characterAbility(i),seed=characterSeedV495(i),rank=charRank(i),rb=rankSkillBonusV512(rank),v=(seed>>>6)%4;
 let table={
  攻撃:[["連撃","追加ダメージ型","combo",1.12,1.00,1.00],["会心","会心強化型","crit",1.16,.98,1.00],["処刑","BREAK追撃型","execute",1.10,1.03,1.00],["強襲","初撃強化型","ambush",1.14,1.00,1.02]],
  妨害:[["封筆","攻撃低下型","seal",.98,1.18,1.05],["錯稿","被ダメ増加型","misprint",1.03,1.12,1.05],["遅筆","行動遅延型","slow",.97,1.15,1.08],["破調","BREAK特化型","break",1.00,1.24,1.02]],
  回復:[["再読","回復強化型","heal",.90,.96,1.12],["余韻","継続回復型","regen",.88,.98,1.14],["浄化","弱体解除型","cleanse",.91,.95,1.10],["慈筆","回復＋回転型","mercy",.92,.97,1.16]],
  支援:[["共著","ゲージ支援型","teamGauge",.94,.98,1.18],["鼓舞","攻撃支援型","buffAtk",.97,1.00,1.14],["護稿","防御支援型","buffDef",.92,1.03,1.14],["連環","回転支援型","chain",.96,1.00,1.20]],
  特殊:[["改稿","攻撃＋BREAK型","rewrite",1.04,1.10,1.10],["転章","奥義回収型","refund",1.00,1.05,1.18],["反転","状態依存型","reverse",1.08,1.08,1.08],["境界","弱点特化型","boundary",1.05,1.12,1.12]],
  防御:[["庇護","軽減特化型","guard",.90,1.02,1.04],["反筆","反撃型","counter",.96,1.04,1.02],["要塞","耐久特化型","fortress",.88,1.08,1.02],["不屈","長期戦型","unyielding",.92,1.05,1.05]],
  速度:[["速筆","ゲージ加速型","fast",1.02,1.00,1.18],["先制","初撃型","first",1.08,1.00,1.12],["連頁","連続行動型","cycle",1.03,1.02,1.20],["疾走","BREAK＋回転型","dash",1.02,1.10,1.16]]
 };
 let b=(table[a.role]||table.特殊)[v];
 return {
  trait:b[0],desc:b[1],code:b[2],
  skillName:`${a.name}・${b[0]}`,
  passiveName:`${rb.label}筆・${b[0]}`,
  skillMul:Number((b[3]*rb.skill).toFixed(3)),
  breakMul:Number((b[4]*rb.break).toFixed(3)),
  epMul:Number((b[5]*rb.ep).toFixed(3)),
...
function abilityDetail(i){
 let a=characterAbility(i),role=a.role,p=characterProfileV495(i),k=characterSkillKitV512(i);
 let base={
  攻撃:`敵へ高威力の文学攻撃。CRIT ${p.crit}%補正。`,
  妨害:`敵の行動を乱し、BREAK補正 ${p.break}%で崩す。`,
  回復:`味方の損耗を回復し、継戦能力を高める。`,
  支援:`味方を強化し、奥義ゲージと文学共鳴を加速。`,
  特殊:`戦況に応じて追加効果が変化する。`,
  防御:`被ダメージを抑え、味方を守る。`,
  速度:`速度 ${p.speed}を活かして行動回転を高める。`
 }[role]||"固有の文学能力。";
 return `${k.skillName}：${base} ${k.desc}。`
}

function ougiInfo(i){
 let a=characterAbility(i),rank=charRank(i),c=C[i]||[],p=characterProfileV495(i),k=characterSkillKitV512(i);
 let names={攻撃:"終章",妨害:"禁書",回復:"再版",支援:"共著",特殊:"改稿",防御:"蔵書",速度:"速筆"};
 let effect={
  攻撃:`敵全体へ特大ダメージ。CRIT補正 ${p.crit}%・ボス追加威力。`,
  妨害:`敵全体を弱体化し、BREAK補正 ${p.break}%・行動遅延。`,
  回復:"味方全体を大回復し、弱体効果を解除。",
  支援:"味方全体の攻撃・防御・奥義回転を強化。",
  特殊:`固有能力を強化再発動し、${p.affinity}属性の追加効果。`,
  防御:"味方全体に軽減効果を付与し、防御態勢を強化。",
  速度:`行動順を加速し、速度 ${p.speed}を威力へ変換。`
 }[a.role]||"固有能力を極限まで解放する。";
 let base=String(c[2]||a.name||"物語解放"),gauge=rank==="UR"?90:rank==="SSR"?100:rank==="SR"?110:120;
 return {name:`${names[a.role]||"奥義"}・${base}【${k.rankLabel}】`,effect:`${effect} 奥義倍率×${k.ougiMul.toFixed(2)}。${k.desc}。`,gauge}
}
...
function favoriteHomePicker(){
 ...
 return `<section class=homePickerV514>
  ...
  <div class=homePickerGridV514>${rows.map(({c,i})=>`<button class="${i===current?"selected":""}" data-home-favorite-v514="${i}"><div class=homePickerArtV514><img src="${characterPortraitV501(i)}" alt="${c[1]}">${rankBadgeV504(i)}</div><div><b>${c[1]}</b><small>${characterAbility(i).role} / Lv.${lv(i)}</small><span>${c[2]}</span></div><em>${i===current?"設定中":"ホームに設定"}</em></button>`).join("")}</div>
 </section>`
}
...
function dedicatedHome(){
 ...
 shell(`<main class=premiumHomeV236>
   ...
   <div class=premiumHomeReadyBadgeV499>V515 HOME</div>
...
function battleDamageV125(kind,i,target){
 ...
 let breakMult=(weak?1.6:1)*((kind==="skill"||kind==="ougi")?k.breakMul:1);
...
function gainEpV125(slot,kind,i=null){
 let b=ensureAdvancedBattleV125(),base={attack:18,skill:10,heal:12,ougi:-100}[kind]||0,mul=i==null?1:characterSkillKitV512(i).epMul;
...
function characterStatProfileV511(i){
 ...
 return {hp:m.hp*rankMul*v(1,.045),atk:m.atk*rankMul*v(4,.055),def:m.def*rankMul*v(7,.055),spd:m.spd*(1+(rankMul-1)*.45)*v(10,.05),power:m.power*rankMul*v(13,.035)}
}
...
function formationPickerV136(slot){
 ...
 return `<div class=formationPickerV500>
   <header class=formationPickerHeadV500>
     <div><small>FORMATION SELECT</small><b>${slot+1}枠目を変更</b><span>${a.length}人 / 入替ボタンで即決定</span></div>
       ...
       return `<article class="formationEasyCardV509 ${i===current?"current":""} ${team.has(i)?"inTeam":""}">
         ...
         <div class=formationEasyMainV509>
           ...
           <div class=formationEasySkillsV509>
             <div><small>固有 / ${characterSkillKitV512(i).trait}</small><b>${characterSkillKitV512(i).skillName}</b><em>${skillRankTextV512(i)}</em></div>
