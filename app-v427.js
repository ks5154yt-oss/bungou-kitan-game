...
try{
  window.__BK_BUILD__="V427";
  localStorage.setItem("bungou_kitan_build","V427");
}catch(_){}
...
function shell(x){
 ...
 A.innerHTML=`${premium?"":`<div class=top><b>文豪綺譚 <span class=gold>V427</span></b><small>EXP ${S.xp} / 🖋️${S.ink}</small></div>`}<div class=screenFade>${x}</div>${premium?"":nav()}`
}
...
function mountBattleV182(){
  ...
  if(ensureBattleStateV120().auto)setTimeout(()=>startAutoCoreV427(),120);
  return markup
}
