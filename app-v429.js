...
try{
  window.__BK_BUILD__="V429";
  localStorage.setItem("bungou_kitan_build","V429");
}catch(_){}
...
function shell(x){
 ...
 A.innerHTML=`${premium?"":`<div class=top><b>文豪綺譚 <span class=gold>V429</span></b><small>EXP ${S.xp} / 🖋️${S.ink}</small></div>`}<div class=screenFade>${x}</div>${premium?"":nav()}`
}
...
function toggleAutoBattleV122(){
 ...
 try{posterAutoSyncV230?.()}catch(_){};setTimeout(()=>placeAutoButtonV429(),0)
}


let __autoButtonRafV429=0;

function removeAutoButtonV429(){
 let x=document.getElementById("autoButtonV429");
 if(x)x.remove();
 if(__autoButtonRafV429){cancelAnimationFrame(__autoButtonRafV429);__autoButtonRafV429=0}
}

function placeAutoButtonV429(){
 let btn=document.getElementById("autoButtonV429");
 let original=document.querySelector(".posterThumbSubV234 [data-battle-auto]");
 if(!btn||!original||!document.body.classList.contains("battleMode"))return;
 ...
 btn.textContent=ensureBattleStateV120().auto?"● AUTO ON":"AUTO";
 btn.classList.toggle("on",!!ensureBattleStateV120().auto);
}

function installAutoButtonV429(){
 removeAutoButtonV429();
 if(!document.body.classList.contains("battleMode"))return;
 let original=document.querySelector(".posterThumbSubV234 [data-battle-auto]");
 if(!original)return;

 // Keep the underlying button as a visual fallback, but let the body-level button own input.
 original.style.pointerEvents="none";

 let btn=document.createElement("button");
 btn.id="autoButtonV429";
 btn.type="button";
 btn.setAttribute("aria-label","AUTO切替");
 btn.setAttribute("aria-pressed",ensureBattleStateV120().auto?"true":"false");
 ...
 const fire=(e)=>{
   ...
   // Toggle exactly once per physical tap.
   toggleAutoBattleV122();

   const auto=!!ensureBattleStateV120().auto;
   btn.setAttribute("aria-pressed",auto?"true":"false");
   btn.textContent=auto?"● AUTO ON":"AUTO";
   btn.classList.toggle("on",auto);
   placeAutoButtonV429();
 };

 // Pointer Events are sufficient on current iOS Safari.
 // Do NOT also listen to touchstart, because one tap can emit both events.
 btn.addEventListener("pointerup",fire,{capture:true});
 btn.addEventListener("click",(e)=>{
   // Keyboard/accessibility fallback only.
   if(e.detail===0)fire(e);
   else{
     e.preventDefault();
     e.stopPropagation();
     if(e.stopImmediatePropagation)e.stopImmediatePropagation();
 ...
 const follow=()=>{
   if(!document.body.classList.contains("battleMode")){
     removeAutoButtonV429();return;
   }
   placeAutoButtonV429();
   __autoButtonRafV429=requestAnimationFrame(follow);
 };
 __autoButtonRafV429=requestAnimationFrame(follow);
}
...
function mountBattleV182(){
  ...
  setTimeout(()=>installAutoButtonV429(),60);if(ensureBattleStateV120().auto)setTimeout(()=>startAutoCoreV427(),120);
  return markup
}
...
function leaveBattleCleanupV191(){removeAutoButtonV429?.();clearPosterQueueV230();posterRendererResetV229();posterStageResetV228();flushBattleSaveV197();battleInvalidateActionsV195();clearTimeout(__battleGovernorTimerV193);__battleLongTaskCountV193=0;
 stopAutoBattleV122?.();
 clearBattleTimersV191();
 purgeBattleTransientDomV191();
 pauseDecorativeFxV190?.(true);
...
function posterThumbSubV234(){
 ...
 return `<div class=posterThumbSubV234>
   <button data-battle-speed="${b.speed===2?1:2}">▶▶ ${b.speed===2?"×2":"×1"}</button>
   <button class="${b.auto?"on":""}" data-battle-auto=1 aria-pressed="${b.auto?"true":"false"}" onclick="event.preventDefault();event.stopPropagation();toggleAutoBattleV122();return false">${b.auto?"AUTO ON":"AUTO"}</button>
