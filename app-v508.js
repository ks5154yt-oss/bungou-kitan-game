...
try{
  window.__BK_BUILD__="V508";
  localStorage.setItem("bungou_kitan_build","V508");
}catch(_){}
...
function shell(x){
 ...
 A.innerHTML=`${premium?"":`<div class=top><b>文豪綺譚 <span class=gold>V508</span></b><small>EXP ${S.xp} / 🖋️${S.ink}</small></div>`}<div class=screenFade>${x}</div>${premium?"":nav()}`
}
     ...
     ${rows.map(({c,i})=>{
       ...
       return `<article class="collectionCardV484 ${c[5]} ${owned?"owned":"unowned"}">
         <button class=collectionMainV484 data-char="${i}">
           <div class=collectionImgV484><img src="${characterImage(c[0])}" alt="">${rankBadgeV504(i)}${S.favs.includes(i)?`<em>★</em>`:""}</div>
...
function transitionAuditV508(){
 let tests=[
   ...
   ["detail",typeof detail==="function"],
   ["cap",typeof cap==="function"],
   ["lv",typeof lv==="function"]
 ];
 ...
 shell(`${uiPageHead("growth")}<main class=growthHubV483>
   ...
   <section class=growthFocusV483>
     ...
     <div class=growthFocusTextV483>
       ...
       <div class=growthNeedV483>${l>=capLv?`現在の上限に到達`:`次Lvまで共有EXP ${need}`}</div>
     </div>
...
function dedicatedHome(){
 ...
 shell(`<main class=premiumHomeV236>
   ...
   <div class=premiumHomeReadyBadgeV499>V508 FIX</div>
...
function charAtk(i){
 ...
 let base=180+lv(i)*18,gear=gearBonus(i)?.atk||0,m=masteryStats(i)?.total||0,sp=gearSpecialization(i)?.bonus||0;
 return Math.round((base+gear*5)*(1+(m+sp)/100))
}
function charDef(i){
 i=Math.max(0,Math.min(C.length-1,Number(i)||0));
 let base=150+lv(i)*15,gear=gearBonus(i)?.def||0,m=masteryStats(i)?.total||0,sp=gearSpecialization(i)?.bonus||0;
 return Math.round((base+gear*3)*(1+(m+sp)/100))
}
function charPower(i){
 i=Math.max(0,Math.min(C.length-1,Number(i)||0));
 return typeof boostedPower==="function"?boostedPower(i):Math.round(700+lv(i)*35)
}
...
function formationIntelV506(i,current){
 ...
 return {
   atk:charAtk(i),def:charDef(i),power:singleCharacterPowerV508(i),
...
function formationPickerV136(slot){
 ...
 let preview=formationPreviewIndexV507(slot,current,a.length?a:[current]);
 return `<div class=formationPickerV500>
   <header class=formationPickerHeadV500><div><small>FORMATION SELECT</small><b>${slot+1}枠目を変更</b><span>${a.length}人 / 候補をタップして比較</span></div><button data-formation-picker-close=1>×</button></header>
   <div class=formationCurrentV500>${(S.sets?.[S.set]||[]).slice(0,6).map((i,n)=>`<span class="${n===slot?"target":""}"><img class=portraitContainV506 src="${characterPortraitV501(i)}" alt="${C[i][1]}"><b>${n+1}</b></span>`).join("")}</div>
   <div class=formationPickerToolsV500><input data-formation-search-v500 value="${S.qol.formationPickQueryV500||""}" placeholder="名前・作品・役割・スキルで検索"><div class=formationRankV500>${["ALL","UR","SSR","SR","R"].map(r=>`<button class="${rank===r?"on":""}" data-formation-rank-v500="${r}">${r==="ALL"?"全員":r}</button>`).join("")}</div><select data-formation-sort-v500><option value=power ${sort==="power"?"selected":""}>戦力順</option><option value=level ${sort==="level"?"selected":""}>Lv順</option><option value=rank ${sort==="rank"?"selected":""}>ランク順</option><option value=name ${sort==="name"?"selected":""}>名前順</option></select></div>
   ${formationPreviewPanelV507(slot,current,preview)}
   <div class=formationPickerListV500>${a.map(i=>{
     let x=formationIntelV506(i,current);
     return `<button class="formationIntelCardV506 ${i===current?"current":""} ${team.has(i)?"inTeam":""} ${i===preview?"previewing":""}" data-formation-preview-v507="${i}">
       <div class=formationIntelPortraitV506><img class=portraitContainV506 src="${characterPortraitV501(i)}" alt="${C[i][1]}">${rankBadgeV504(i)}</div>
       <div class=formationIntelBodyV506>
         <div class=formationIntelNameV506><div><small>${x.role} / ${bundanTag(i)}</small><b>${C[i][1]}</b><span>${C[i][2]}</span></div><strong>Lv.${lv(i)}</strong></div>
         <div class=formationIntelStatsV506><span>戦力<b>${x.power.toLocaleString()}</b><em class="${x.cmp.power>0?"up":x.cmp.power<0?"down":""}">${signedV506(x.cmp.power)}</em></span><span>攻撃<b>${x.atk}</b><em class="${x.cmp.atk>0?"up":x.cmp.atk<0?"down":""}">${signedV506(x.cmp.atk)}</em></span><span>防御<b>${x.def}</b><em class="${x.cmp.def>0?"up":x.cmp.def<0?"down":""}">${signedV506(x.cmp.def)}</em></span></div>
         <div class=formationIntelSkillV506><span><small>固有</small><b>${x.ability}</b></span><span><small>奥義 ${x.gauge}</small><b>${x.ougi}</b></span></div>
       </div>
       ${team.has(i)?`<i class=formationInTeamV506>編成中</i>`:""}
     </button>`
   }).join("")}</div>
 </div>`
}
...
let fpv507=e.target.closest("[data-formation-preview-v507]");
if(fpv507){
 window.__formationPreviewV507=+fpv507.dataset.formationPreviewV507;
 let ov=document.getElementById("formationPickerOverlay");
 if(ov)ov.querySelector(".resultCard").innerHTML=formationPickerV136(Number(ov.dataset.slot||0));
 return
}
let fa507=e.target.closest("[data-formation-apply-v507]");
if(fa507){
 let [s,i]=fa507.dataset.formationApplyV507.split(":").map(Number);
 return setFormationSlotV136(s,i)
}
