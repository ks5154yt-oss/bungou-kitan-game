...
try{
  window.__BK_BUILD__="V501";
  localStorage.setItem("bungou_kitan_build","V501");
}catch(_){}
...
function shell(x){
 ...
 A.innerHTML=`${premium?"":`<div class=top><b>文豪綺譚 <span class=gold>V501</span></b><small>EXP ${S.xp} / 🖋️${S.ink}</small></div>`}<div class=screenFade>${x}</div>${premium?"":nav()}`
}
...
function showGachaResults(got,cost){let d=document.createElement("div");d.className="resultOverlay gachaResultOverlayV500";let high=got.filter(i=>["SR","SSR","UR"].includes(C[i][5])).length;let top=got.reduce((best,i)=>charRankOrder(charRank(i))>charRankOrder(charRank(best))?i:best,got[0]??0);d.innerHTML=`<div class="resultPanel gachaResultPanelV500"><div class=gachaResultHeadV500><div><small>SUMMON RESULT</small><h1>召喚結果</h1></div><button data-gacha-close=1>×</button></div><div class=resultGrid><div>召喚<b>${got.length}</b></div><div>SR以上<b>${high}</b></div><div>最高<b>${charRank(top)}</b></div></div><div class=gachaResultGridV500>${got.map(i=>{let c=C[i],sig=characterSignatureV496(i);return `<button class="gachaMiniV500 ${charRank(i)}" data-gacha-char-v500="${i}"><div class=gachaMiniImgV500><img src="${characterImage(c[0])}"><span>${charRank(i)}</span></div><div class=gachaMiniTextV500><small>${characterAbility(i).role} / ${bundanTag(i)}</small><b>${c[1]}</b><span>${characterAbility(i).name}</span><em>${sig.title}</em></div></button>`}).join("")}</div><div class=gachaHintV500>タップでキャラ詳細</div><div class="gachaActions gachaActionsV500"><button class=btn data-gacha-repeat="${got.length}">もう一度</button><button class=btn data-gacha-close=1>召喚へ戻る</button></div></div>`;document.body.appendChild(d)}
...
function collectionOwnedV484(i){return (S.dupes?.[i]||0)>0||lv(i)>1||S.sets?.some(a=>a.includes(i))}
...
function list(){
 ...
 shell(`${uiPageHead("list")}<main class=collectionHubV484>
   <section class=collectionHeroV484>
     <div><small>LITERARY ARCHIVE</small><h1>文豪蒐集</h1><p>所持、育成、固有能力、装備適性を一画面で確認。</p></div>
     ...
     ${rows.map(({c,i})=>{
       ...
       return `<article class="collectionCardV484 ${c[5]} ${owned?"owned":"unowned"}">
         <button class=collectionMainV484 data-char="${i}">
           <div class=collectionImgV484><img src="${characterImage(c[0])}" alt=""><span>${c[5]}</span>${S.favs.includes(i)?`<em>★</em>`:""}</div>
           <div class=collectionInfoV484>
             <small>${role} / ${c[3]}</small><h2>${c[1]}</h2><p>《${c[2]}》</p>
             ...
             <div class=collectionAbilityV484><b>${a.name}</b><span>${abilityDetail(i)}</span></div>
...
function summonPoolV480(rank){let a=C.map((c,i)=>i).filter(i=>C[i][5]===rank);return a.length?a:C.map((c,i)=>i)}
...
function systemPanelV490(){
 ...
 shell(`${uiPageHead("home")}<main class=systemPanelV490>
   ...
   <section class=systemRuntimeV491>
     ...
     <div class=systemRuntimeListV491>
       ${runtimeLogListV491().length?runtimeLogListV491().slice(0,5).map(x=>`<div><span>${x.type}</span><b>${new Date(x.at).toLocaleTimeString("ja-JP",{hour:"2-digit",minute:"2-digit"})}</b><small>${x.msg}</small></div>`).join(""):`<div class=runtimeOkV491>異常ログなし</div>`}
     </div>
   </section>
...
function growthSelectedV483(){
 ...
 if(!Number.isInteger(i)||i<0||i>=C.length)i=S.sets?.[S.set]?.[0]??0;
 S.qol=S.qol||{};S.qol.growthSelectedV483=i;return i
}
function growthExpNeedV483(i){
 let l=lv(i),cap=cap(i);if(l>=cap)return 0;
...
function growthBreakCostV483(i){
 let cap=cap(i);
 return {gold:cap<80?2500:cap<110?4500:7000,mat:cap<80?12:cap<110?22:36}
}
...
function growthLevelOneV483(i){
 ...
 S.xp-=need;S.levels[i]=Math.min(cap(i),lv(i)+1);return true
}
...
function growth(){
 ...
 let i=growthSelectedV483(),c=C[i],l=lv(i),cap=cap(i),need=growthExpNeedV483(i),avg=growthAvgBonusV483(),role=growthRoleV483(i),br=growthBreakCostV483(i);
 shell(`${uiPageHead("growth")}<main class=growthHubV483>
   <section class=growthHeroV483>
     <img src="${characterPortraitV501(i)}" alt="${c[1]}">
   ...
   <section class=growthRosterV483>
     ${growthPortraitsV483().map(n=>`<button class="${n===i?"active":""}" data-growth-char-v483="${n}"><img src="${characterPortraitV501(n)}" alt="${C[n][1]}"><span>${C[n][5]}</span><b>${C[n][1]}</b><small>Lv.${lv(n)}</small></button>`).join("")}
   </section>

   <section class=growthFocusV483>
     <div class=growthPortraitV483><img src="${characterPortraitV501(i)}" alt="${c[1]}"><span>${c[5]}</span></div>
     <div class=growthFocusTextV483>
       <small>${role} / ${c[3]}</small><h2>${C[i][1]}</h2><p>${c[2]}</p>
       <div class=growthLevelLineV483><b>Lv.${l}</b><span>/ ${cap}</span></div>
       <div class=growthBarV483><i style="width:${cap?Math.min(100,l/cap*100):0}%"></i></div>
       <div class=growthNeedV483>${l>=cap?`現在の上限に到達`:`次Lvまで共有EXP ${need}`}</div>
     </div>
     <div class=growthStatBoxV483>
       <span>攻撃<b>${Math.round(charAtk(i)*(1+avg.bonus/100))}</b></span>
       <span>防御<b>${Math.round(charDef(i)*(1+avg.bonus/100))}</b></span>
       <span>戦力<b>${charPower(i).toLocaleString()}</b></span>
     </div>
   </section>

   <section class=growthActionsV483>
     <button data-growth-up-v483="1" ${(l>=cap||(S.xp||0)<need)?"disabled":""}><b>Lv +1</b><span>EXP ${need||"-"}</span></button>
     <button data-growth-up-v483="5" ${(l>=cap)?"disabled":""}><b>Lv +5</b><span>最大 ${growthBatchCostV483(i,l+5)}</span></button>
     <button data-growth-up-v483="10" ${(l>=cap)?"disabled":""}><b>Lv +10</b><span>最大 ${growthBatchCostV483(i,l+10)}</span></button>
     <button class=growthMaxV483 data-growth-max-v483=1 ${(l>=cap)?"disabled":""}><b>上限まで</b><span>所持EXP内</span></button>
   </section>

   <section class=growthBreakV483>
     <div><small>LIMIT BREAK</small><h2>上限解放</h2><p>${cap>=150?"最大上限150に到達":growthCanBreakV483(i)?`文銭 ${br.gold} / 資料 ${br.mat}`:`Lv.${cap}到達で解放可能`}</p></div>
...
function characterDossierV495(i){
 ...
 return `<section class=characterDossierV495>
   ...
   <div class=characterDossierGridV495>
     <div><small>固有能力</small><b>${a.name}</b><span>${abilityDetail(i)}</span></div>
...
function abilityDetail(i){
 let a=characterAbility(i),role=a.role,p=characterProfileV495(i);
 let desc={
  "攻撃":`${a.name}：敵へ高威力の文学攻撃。CRIT ${p.crit}%補正を持ち、追撃で畳み掛ける。`,
  "妨害":`${a.name}：敵の行動を乱し、攻撃性能を低下。BREAK補正 ${p.break}%で崩しを狙う。`,
  "回復":`${a.name}：味方の損耗を回復し、WAVEをまたぐ継戦能力を高める。`,
  "支援":`${a.name}：味方全体を強化し、奥義ゲージと文学共鳴を加速する。`,
  "特殊":`${a.name}：戦況に応じて効果が変化する改稿型の固有能力。`,
  "防御":`${a.name}：被ダメージを抑え、味方を守りながら反撃機会を作る。`,
  "速度":`${a.name}：速度 ${p.speed}を活かして先手を取り、行動回転を高める。`
 }[role]||`${a.name}：この文豪固有の文学能力。`;
 return desc
}
function ougiInfo(i){
 let a=characterAbility(i),rank=charRank(i),c=C[i]||[],p=characterProfileV495(i);
 let names={攻撃:"終章",妨害:"禁書",回復:"再版",支援:"共著",特殊:"改稿",防御:"蔵書",速度:"速筆"};
 let effects={
  攻撃:`敵全体へ特大ダメージ。CRIT補正 ${p.crit}%、ボスへ追加威力。`,
  妨害:`敵全体の攻撃と行動効率を低下。BREAK補正 ${p.break}%で次行動を遅延。`,
  回復:"味方全体を大回復し、弱体効果を1つ解除。WAVE継続時も小回復。",
  支援:"味方全体の攻撃・防御を強化し、奥義ゲージを加速。",
  特殊:`固有能力を強化再発動。${p.affinity}属性の追加効果で戦況を書き換える。`,
  防御:"味方全体に軽減効果を付与し、被弾後に反撃支援。",
  速度:`行動順を加速し、敵全体へ高速連撃。速度 ${p.speed}を威力へ変換。`
 };
 let base=String(c[2]||a.name||"物語解放");
 return {name:`${names[a.role]||"奥義"}・${base}`,effect:effects[a.role]||"固有能力を極限まで解放する。",gauge:rank==="UR"?100:rank==="SSR"?110:120}
}
...
function battleSkillCutin(i,type="skill"){
 ...
 let hold=type==="ougi"?(auto?480:720):(auto?260:420);
...
function dedicatedHome(){
 ...
 shell(`<main class=premiumHomeV236>
   ...
   <div class=premiumHomeReadyBadgeV499>V501 FIX</div>
...
function battleDamageV125(kind,i,target){
 ...
 mult*=statusModifierV127(target).damageTaken*comboMultiplierV129();let dmg=Math.max(1,Math.round(base*mult));
...
function formationPickerV136(slot){
 ...
 let current=(S.sets?.[S.set]||[])[slot],team=new Set(S.sets?.[S.set]||[]);
 let a=C.map((_,i)=>i).filter(i=>{let hit=!q||String(C[i][1]).toLowerCase().includes(q)||String(C[i][2]).toLowerCase().includes(q)||String(characterAbility(i).role).toLowerCase().includes(q);return hit&&(rank==="ALL"||charRank(i)===rank)});
 a.sort((x,y)=>sort==="level"?lv(y)-lv(x)||unitPower(y)-unitPower(x):sort==="rank"?charRankOrder(charRank(y))-charRankOrder(charRank(x))||unitPower(y)-unitPower(x):sort==="name"?String(C[x][1]).localeCompare(String(C[y][1]),"ja"):unitPower(y)-unitPower(x));
 return `<div class=formationPickerV500><header class=formationPickerHeadV500><div><small>FORMATION SELECT</small><b>${slot+1}枠目を変更</b><span>${a.length}人 / 専用立ち絵対応</span></div><button data-formation-picker-close=1>×</button></header>
 <div class=formationCurrentV500>${(S.sets?.[S.set]||[]).slice(0,6).map((i,n)=>`<span class="${n===slot?"target":""}"><img src="${characterPortraitV501(i)}" alt="${C[i][1]}"><b>${n+1}</b></span>`).join("")}</div>
 <div class=formationPickerToolsV500><input data-formation-search-v500 value="${S.qol.formationPickQueryV500||""}" placeholder="名前・作品・役割で検索"><div class=formationRankV500>${["ALL","UR","SSR","SR","R"].map(r=>`<button class="${rank===r?"on":""}" data-formation-rank-v500="${r}">${r==="ALL"?"全員":r}</button>`).join("")}</div><select data-formation-sort-v500><option value=power ${sort==="power"?"selected":""}>戦力順</option><option value=level ${sort==="level"?"selected":""}>Lv順</option><option value=rank ${sort==="rank"?"selected":""}>ランク順</option><option value=name ${sort==="name"?"selected":""}>名前順</option></select></div>
 <div class=formationPickerListV500>${a.map(i=>`<button class="${i===current?"current":""} ${team.has(i)?"inTeam":""}" data-formation-pick="${slot}:${i}"><img src="${characterPortraitV501(i)}" alt="${C[i][1]}"><div><span>${charRank(i)} / ${characterAbility(i).role}</span><b>${C[i][1]}</b><small>${C[i][2]}</small></div><aside><b>Lv.${lv(i)}</b><span>${unitPower(i).toLocaleString()}</span>${team.has(i)?`<em>編成中</em>`:""}</aside></button>`).join("")}</div></div>`
}
function openFormationPickerV136(s){document.getElementById("formationPickerOverlay")?.remove();let o=document.createElement("div");o.id="formationPickerOverlay";o.className="resultOverlay formationPickerOverlayV500";o.dataset.slot=String(s);o.innerHTML=`<div class=resultCard>${formationPickerV136(s)}</div>`;document.body.appendChild(o)}
...
let c=e.target.closest("[data-char]");if(c)return detail(+c.dataset.char);let howClose=e.target.closest("[data-how-close]");if(howClose){let o=e.target.closest(".resultOverlay");if(o)o.remove();return}
...
let bt=e.target.closest("[data-battle]");if(bt){if(!stageTapGuard(bt))return;let ch=+bt.dataset.battle,mode=bt.dataset.mode||"normal";if(ch>0&&!(S.progress.clears[ch-1]>0))return toast("前の章をクリアすると解放されます");if(mode==="hard"&&!hardUnlocked(ch))return toast("NORMALを3回クリアでHARD解放");return startBattleV180(ch,mode)}});
