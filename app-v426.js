...
try{
  window.__BK_BUILD__="V426";
  localStorage.setItem("bungou_kitan_build","V426");
}catch(_){}
...
function shell(x){
 ...
 A.innerHTML=`${premium?"":`<div class=top><b>文豪綺譚 <span class=gold>V426</span></b><small>EXP ${S.xp} / 🖋️${S.ink}</small></div>`}<div class=screenFade>${x}</div>${premium?"":nav()}`
}
...
function battleSkillCutin(i,type="skill"){
 ...
 x.innerHTML=`<img src="${characterImage(C[i][0])}"><div><small>${type==="ougi"?"奥義":"固有能力"}</small><b>${type==="ougi"?o.name:a.name}</b><span>${C[i][1]}</span></div>`;r.appendChild(x);setTimeout(()=>x.classList.add("show"),20);setTimeout(()=>{x.classList.remove("show");setTimeout(()=>x.remove(),350)},type==="ougi"?1500:1000)
}
...
let __nativeAutoPumpV425=null;
let __nativeAutoLastProgressV425=0;
let __nativeAutoLastSignatureV425="";

function stopNativeAutoPumpV425(){
 if(__nativeAutoPumpV425){clearInterval(__nativeAutoPumpV425);__nativeAutoPumpV425=null}
}

function autoSignatureV425(){
 let b=ensureBattleStateV120();
 let hp=(b.enemyHp||[]).reduce((a,x)=>a+(+x||0),0);
 return [b.wave||0,b.turn||0,b.unit||0,hp].join(":")
}

function syncNativeAutoUiV425(){
 let b=ensureBattleStateV120();
 ...
 refreshCleanCommandStateV241?.();
}

function startNativeAutoPumpV425(){
 stopNativeAutoPumpV425();
 __nativeAutoLastSignatureV425=autoSignatureV425();
 __nativeAutoLastProgressV425=performance.now();

 const tick=()=>{
   let b=ensureBattleStateV120();
   if(!b.auto||!document.body.classList.contains("battleMode")){
     stopNativeAutoPumpV425();return;
   }

   syncNativeAutoUiV425();

   const sig=autoSignatureV425();
   if(sig!==__nativeAutoLastSignatureV425){
     __nativeAutoLastSignatureV425=sig;
     __nativeAutoLastProgressV425=performance.now();
   }

   const idle=performance.now()-__nativeAutoLastProgressV425;

   // Recover from stale busy locks or a lost timeout.
   if(idle>1800){
     if(autoBattleTimerV122){clearTimeout(autoBattleTimerV122);autoBattleTimerV122=null}
     if(autoBattleBusyV122||__battleActionBusyV195){
       autoBattleBusyV122=false;
   ...
   if(!autoBattleBusyV122&&!__battleActionBusyV195&&!autoBattleTimerV122){
     runAutoBattleV122();
   }
 };

 __nativeAutoPumpV425=setInterval(tick,250);
 setTimeout(tick,60);
}

function toggleAutoBattleV122(){
 let b=ensureAdvancedBattleV125();
 b.auto=!b.auto;
 queueBattleSaveV197(true);
 try{syncNativeAutoUiV425?.()}catch(_){}
 try{posterAutoSyncV230?.()}catch(_){}
 if(b.auto){
   stopAutoBattleV122();
   try{stopNativeAutoPumpV425?.()}catch(_){}
   autoBattleBusyV122=false;
   try{battleInvalidateActionsV195()}catch(_){}
   startAutoDirectV426();
   toast("AUTO ON");
 }else{
   stopAutoDirectV426();
...
function resetBattleV126(){
 stopAutoBattleV122();S.battleV120={wave:1,turn:1,speed:1,auto:false,target:0,unit:0,enemyHp:[100,100,100],ep:[35,35,35,35,35],breakGauge:[100,100,100],enemyStatus:[[],[],[]],allyHp:[100,100,100,100,100],combo:0,enemyRosterWave:0,log:[]};save();persistentSaveWrite();document.querySelector(".gameOverV126")?.remove();return cinematicBattleV119()
}
...
function battleTopBarV151(){
 let b=ensureAdvancedBattleV125();
 return `<div class=battleTopBarV151><div><small>第${b.wave}章　言葉のはじまり</small><span>WAVE ${b.wave}/3　 TURN ${b.turn}</span></div><nav><button data-battle-speed="${b.speed===2?1:2}">▶▶ ${b.speed===2?"×2":"×1"}</button><button class="${b.auto?"on":""}" data-battle-auto=1>${b.auto?"● AUTO":"AUTO"}</button>${lightBattleButtonV187()}${battleQualityLabelV192()}<button data-snappy-v204=1>演出</button><button data-battle-pause=1>Ⅱ</button></nav></div>`
}
...
function mountBattleV182(){
  ...
  if(ensureBattleStateV120().auto)setTimeout(()=>startNativeAutoPumpV425(),120);
  return markup
}
...
function leaveBattleCleanupV191(){clearPosterQueueV230();posterRendererResetV229();posterStageResetV228();flushBattleSaveV197();battleInvalidateActionsV195();clearTimeout(__battleGovernorTimerV193);__battleLongTaskCountV193=0;
 stopAutoBattleV122?.();
 clearBattleTimersV191();
 purgeBattleTransientDomV191();
 pauseDecorativeFxV190?.(true);
...
function posterThumbSubV234(){
 ...
 return `<div class=posterThumbSubV234>
   <button data-battle-speed="${b.speed===2?1:2}">▶▶ ${b.speed===2?"×2":"×1"}</button>
   <button class="${b.auto?"on":""}" data-battle-auto=1>${b.auto?"AUTO ON":"AUTO"}</button>
...
document.addEventListener("pointerup",function(e){
 let ba=e.target.closest?.("[data-battle-auto]");
 if(!ba)return;
 if(ba.dataset.v426Handled==="1"){ba.dataset.v426Handled="";return}
 ba.dataset.v426Handled="1";
 e.preventDefault();e.stopImmediatePropagation();
 toggleAutoBattleV122();
 setTimeout(()=>{try{ba.dataset.v426Handled=""}catch(_){}},80);
},true);
...
document.addEventListener("click",function(e){
 ...
 if(!ba)return;
 e.preventDefault();e.stopImmediatePropagation();
 // pointerup already handles touch/pointer input. Keyboard-generated click falls back here.
 if(e.detail===0)return toggleAutoBattleV122();
},true);
...
let sdAction=e.target.closest("[data-sd-action]");if(sdAction){let b=ensureAdvancedBattleV125();if(sdAction.dataset.sdAction==="ougi"&&!canOugiV125(b.unit||0)){battleTextV121("EP不足","warning",50,55);return}if(b.auto){battleTextV121("AUTO","warning",50,55);return}return battleActionV120(sdAction.dataset.sdAction)}
